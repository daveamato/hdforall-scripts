...
$out="";
$srt==array();
$link11="http://uphero.xpresso.eu/srt/m/glob.php";
$html=file_get_contents($link11);
$videos = explode(",", $html);
...
for ($k=0;$k<count($t);$k++) {
  $m=explode("|",$t[$k]);
  if ($imdbid == $m[1]) {
        $srt="http://uphero.xpresso.eu/srt/m/".$m[0].".srt";
        ...
        movie=geturl(url);
        dummy=getURL("http://127.0.0.1/cgi-bin/scripts/util/srt_xml.php?file='.$srt.'");';
        echo '
        cancelIdle();
 ...
 $l="http://superchillin.com/api/cipac/series_dump.php";
  ...
  if (strpos($h1,$find) !== false) {
   ...
   if ($id_s) {
     ...
     $z=json_decode($h2,1);
     foreach ($z as $key => $val) {
      if ($z[$key]["season"] == intval($sezon,10) && $z[$key]["episode"] == intval($episod,10)) {
        $id_ep= $z[$key]["id"];
        $ep_tit= $z[$key]["title"];
        break;
      }
   ...
   if ($id_ep) {
    $link22="http://uphero.xpresso.eu/srt/s/glob.php";
    $html=file_get_contents($link22);
    if (strpos($html,",".$id_ep.".srt") !== false) {
        $srt="http://uphero.xpresso.eu/srt/s/".$id_ep.".srt";
        $f = "/usr/local/bin/home_menu";
	    echo'
	    <item>
